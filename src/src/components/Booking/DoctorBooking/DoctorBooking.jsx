...
import { useCreateAppointmentMutation , useGetAppointmentsByDoctorIdQuery} from '../../../redux/api/appointmentApi';
...
if (dIsLoading) {
...
} else if (dIsError || !time) {
    dContent = <div>Something went wrong!</div>;
} else {
    // Filter out the booked time slots for the selected date
    let availableTimeSlots = time.filter(slot => {
        return !doctorAppointments.some(appointment => {
            return moment(appointment.scheduleDate).format('YYYY-MM-DD') === moment(selectedDate).format('YYYY-MM-DD') && appointment.scheduleTime === slot.slot.time;
        });
    });
    ...
    dContent = (
        <>
            {availableTimeSlots.length === 0 ? (
                <Empty children="Doctor Is not Available" />
            ) : (
                availableTimeSlots.map((item, id) => (
    ...
    const handleConfirmSchedule = () => {
        const selectedDateWithoutTime = moment(selectedDate).format('YYYY-MM-DD');
    
        const isDateAvailable = doctorAppointments.every(appointment => {
            const appointmentDateWithoutTime = moment(appointment.scheduleDate).format('YYYY-MM-DD');
            return appointmentDateWithoutTime !== selectedDateWithoutTime;
        });
    
        if (!isDateAvailable) {
            message.error("Appointments are already booked for the selected date. Please choose another date.");
            return;
        }
    ...
    // Fetch and log doctor appointments
    const { data: doctorAppointmentsData, refetch: refetchAppointments, isLoading: isAppointmentsLoading } = useGetAppointmentsByDoctorIdQuery(doctorId, { skip: !doctorId });

    useEffect(() => {
        if (doctorAppointmentsData) {

            doctorAppointmentsData.forEach(appointment => {
